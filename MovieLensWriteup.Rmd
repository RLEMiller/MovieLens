...
```{r ProvidedParser, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r createTestAndTrain, eval = TRUE, echo=TRUE, cache=TRUE, warning=FALSE, message=FALSE}
...
```{r headTrain, eval = TRUE, echo = TRUE}
  head(train,3)
```
...
```{r userIdcheck, eval=TRUE, echo=TRUE}
  str(train$userId)
```
...
```{r genresCheck, echo = FALSE, cache=TRUE}
...
```{r WranglingFunction, echo=TRUE, warning=FALSE, message=FALSE}
...
```{r genrePrev, echo=FALSE, cache=TRUE, fig.height=3, fig.cap="(A) Prevalence of Genres in train dataset.  (B) Histogram of Release Year"}
...
```{r movieUserCountHist, echo=FALSE, fig.height=3, fig.cap="Histograms of the number of ratings for each movie and user, respectively."}
...
```{r RegularizationJustificationAnalysis, cache=TRUE}
...
```{r regularizationMovie, echo=TRUE, warning=FALSE, message=FALSE}
...
```{r regularizationMovieandUser, echo=FALSE, cache=TRUE, message=FALSE, fig.height=3, fig.cap="Regularization curves for movie effect and user effect."}
...
```{r OverallRegularized, echo=FALSE}
...
```{r SigmaVarsExplained, echo=FALSE}
...
```{r SigmoidalClippingMovieUser, message=FALSE, fig.height=3, fig.cap="Regularization with simple clipping and sigmoid clipping on the user and movie effect model."}
...
```{r SigmoidClippingResults, echo=FALSE}
...
```{r AddBuBiTrain, echo=FALSE, message=FALSE}
...
```{r tempAge, echo=FALSE, fig.height=4, fig.cap="Average rating for movies by release year and by age of movie at the time of rating."}
...
```{r PlotAgeByMovieEffect, echo = FALSE, fig.height=4, fig.cap="Rating averages grouped by both movie age and user."}
  suppressWarnings(set.seed(1234,sample.kind = "Rounding"))
...
```{r AgeEffectByMovieAndAge, cache=TRUE, echo=FALSE}
  b_its <- train %>%
     mutate( age = year(as_datetime(timestamp)) - year) %>%
...
```{r AgeEffectByMovieAndAgeRegularized, echo = FALSE, message=FALSE, warning=FALSE, error=FALSE, fig.height=3, fig.cap="Regularization curve for movie age effect, grouped by user and age of movie at time of rating."}
...
```{r AgeEffectByMovieAndAgeRegularizedClipped, echo=FALSE}  
...
```{r circadianEffect, echo = FALSE, fig.height=4, fig.cap="Average rating by time of day for all users. The total number of ratings at that hour of day is given by the point size. Note that the y axis only covers plus or minus 0.1 rating points. This suggests little predictive power in time of day."}
...
```{r circadianByUserPlot, echo = FALSE, fig.height=4, fig.cap="Average ratings by hour of day for several prolific raters. Rating patterns are not uniform across users."}
...
```{r circadianByUserb_ut, echo=FALSE}
...
```{r regularizeCircadianByUser, echo = FALSE, fig.height=3, fig.cap="Regularization curve for circadian effect grouped by user."}
...
```{r driftUserTime, warning=FALSE, echo=FALSE, fig.height=4, fig.cap="Drift in user rating over time, by week, for randomly sampled users."}
...
```{r drift500Users, warning=FALSE, echo = FALSE, fig.height=3, fig.cap="Cumulative drift for 500 randomly selected users."}
...
```{r genresRawAvg, echo=FALSE, warning=FALSE, fig.height = 4, fig.cap="Raw genres as numeric factor vs. the average residual per user for a small random set of prolific users. Some clustering becomes apparent, but there are still far too many variables for this method to be human-readable."}
  suppressWarnings(set.seed(1234,sample.kind = "Rounding"))
...
```{r calculateFirstb_ug, echo=FALSE, error=FALSE, message=FALSE,warning=FALSE}
...
```{r regularizeGenreEffect, fig.height=3, fig.cap="Regularization curve for genre effect based on factorized original genres string."}
...
```{r residualByGenreFiveUsers, echo=FALSE, warning=FALSE, message=FALSE, fig.height=5, fig.cap="Average residual rating for each genre for five randomly selected users."}
  set.seed(1234,sample.kind = "Rounding")
...
```{r GenreRegularization, echo=FALSE, warning=FALSE, message=FALSE, cache=TRUE, fig.height=3, fig.cap="Binarized Genre Regularization"}
  lambdas <- seq(0,20,5)
  regularized_rmses <- sapply(lambdas, function(L){
...
```{r Genres, message=FALSE, warning=FALSE, fig.cap="Heatmap of binarized genres average rating for several randomly chosen users. Binarized genres with near zero variance are removed and users and genres are clustered by distance. Red color indicates a negative residual and blue indicates a positive residual."}
...
```{r RecommenderGenres, cache=TRUE, message=FALSE, error=FALSE}
...
```{r SVDFtrainingTerms, echo = FALSE, cache=TRUE}
...
```{r AssignSVDFtrainingterms, cache=TRUE}
...
```{r AddGenreEffectToTestTrainResidual, eval = TRUE}
...
```{r MovieUserInteractions, cache=TRUE, warning=FALSE, message=FALSE}
...
```{r FullSVD, message=FALSE, error=FALSE}
...
```{r TrainbuiSVD, cache=TRUE, eval = TRUE, warning=FALSE, error=FALSE, fig.height=3, fig.cap="Regularization Curve for SVD number of matrix factor components to minimize RMSE."}
...
```{r EffectsInBlend, echo = FALSE, cache=TRUE}
...
```{r edxReconstitution, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r TrainFullModel, echo=FALSE, cache=TRUE, message=FALSE, warning=FALSE}
...
```{r crossValidateFinal, cache=TRUE, message=FALSE, warning=FALSE}
...
```{r buildLinearPredictionPieces, echo = FALSE, cache=TRUE}
...
```{r ListOfParams, echo=FALSE, cache=TRUE}
...
```{r edxSVDF, cache=TRUE, echo=TRUE, message=FALSE, error=FALSE}
...
```{r SVDFull, echo=TRUE, message=FALSE, warning=FALSE,error=FALSE}
...
```{r Train_b_ui_SVDFinal, eval = TRUE, cache=TRUE}
