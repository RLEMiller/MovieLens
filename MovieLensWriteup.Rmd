...
```{r circadianEffect, echo = FALSE, fig.height=3, fig.cap="Average rating by time of day for all users."}
...
```{r driftUserTime, warning=FALSE, echo=FALSE, fig.height=3, fig.cap="Drift in user rating over time, by week, for randomly sampled users."}
...
```{r genresRawAvg, echo=FALSE, warning=FALSE, fig.height = 3, fig.cap="Raw genres as numeric factor vs. the average residual per user for a small random set of prolific users. Some clustering becomes apparent, but there are still far too many variables for this method to be human-readable."}
...
```{r TrainFullModel, echo=FALSE, cache=TRUE}
...
```{r crossValidateFinal, cache=TRUE}
