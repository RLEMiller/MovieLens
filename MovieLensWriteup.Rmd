...
We can also examine how ratings change with the age of the movie. If we find the average ratings of all movies of the same age in the dataset, we get the plot in Figure \ref{fig:tempAge}.  

```{r tempAge, echo=FALSE, fig.height=3, fig.cap="Average rating for movies by release year and by age of movie at the time of rating."}
...
```{r PlotAgeByMovieEffect, echo = FALSE, fig.height=3, fig.cap="Rating averages grouped by both movie age and user."}
  suppressWarnings(set.seed(1234,sample.kind = "Rounding"))
...
```{r circadianByUserPlot, echo = FALSE, fig.height=3, fig.cap="Average ratings by hour of day for several prolific raters. Rating patterns are not uniform across users."}
...
```{r residualByGenreFiveUsers, echo=FALSE, warning=FALSE, message=FALSE, fig.height=4, fig.cap="Average residual rating for each genre for five randomly selected users."}
  set.seed(1234,sample.kind = "Rounding")
...
```{r CalcBuiSVDTrained, cache=TRUE, warning=FALSE, error=FALSE}
  ...
  for( j in 1:nrow(test)) buiSVD[j] <- 
        sum( rec@model$svd$u[test$U_index[j], 1:K_User_Movie_SVD ]  * 
               rec@model$svd$d[1:K_User_Movie_SVD] *
               rec@model$svd$v[test$V_index[j], 1:K_User_Movie_SVD ])
  ...
  test <- test %>%
    mutate(rating_hat = mu + b_i + b_u + 
               ...
               nafill(b_ug, fill = 0) +
               nafill(b_ui, fill = 0)) %>%
    mutate(rating_hat = SimpleClip(rating_hat, UB=5.0, LB=0.5))
  
  # What was our loss function based on this prediction?
  RMSE(test$rating, test$rating_hat)
```

The final results of the model development are in Table \ref{tab:resultsBeforeFinal}.  

```{r resultsBeforeFinal, echo=FALSE, cache=TRUE, warning=FALSE, error=FALSE}
