...
#' 
#' ## Regularization
#' The regularization scheme used in the BellKor solution and in the coursework penalized large values of $b_{u}$ and $b_{i}$ when the corresponding number of ratings was small. The coursework minimized $RMSE$ for both $\hat{b}_u$ and $\hat{b}_{i}$ simultaneously. We will decouple the user and movie effects, as seen in the following figure, in part because we will want to perform some regularization on other estimates for additional effects and it will become very cumbersome to try to regularize all of the terms simultaneously.  
#' 
...
#' 
#' We then calculate our prediction. Because of the size of the output matrix, we do this element-wise and only calculate a prediction for a user/movie pair found in the `test` set. The figure below shows the regularization curve used to minimize $RMSE$ by the number of matrix factor components, or k's.  
#' 
